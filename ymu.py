...
open_in_browser_button = ctk.CTkLabel(
    master=tabview.tab(check_if_dll_is_downloaded()),
    text="↣ Open YimMenu's GitHub-Repo [Browser] ↢",
    text_color=WHITE,
    font=CODE_FONT,
...
open_in_browser_button.bind("<ButtonRelease-1>", open_source_ib)
...
def fsl_hover(e):
    open_fsl_in_browser_button.configure(
        text="Open FSL's UC-Thread [Browser] ↗",
        font=CODE_FONT_U,
        text_color=GREEN,
        cursor="hand2",
    )
...
open_fsl_in_browser_button = ctk.CTkLabel(
    master=tabview.tab(check_if_dll_is_downloaded()),
    text="↣ Open FSL's UC-Thread [Browser] ↢",
    text_color=WHITE,
    font=CODE_FONT,
...
open_fsl_in_browser_button.pack(pady=10, padx=0, expand=False, fill=None)
open_fsl_in_browser_button.bind("<Enter>", fsl_hover)
open_fsl_in_browser_button.bind("<Leave>", fsl_normal)
open_fsl_in_browser_button.bind("<ButtonRelease-1>", open_fsl_ib)
...
def get_rgl_path() -> str:
    ...
    try:
        ...
        logger.error(
            f"An error has occured while trying to read RGL path! Traceback: {err}",
            exc_info=1,
        )
        pass
...
def start_gta():
    ...
    if not is_running:
        try:
            ...
            if run_cmd == "rgs":
                inject_progress_label.configure(
                    text="Please wait while YMU attempts to launch your game through\nRockstar Games Launcher..."
                )
                logger.warning(
                    "Rockstar Games Launcher-Option may not work for some users. If thats the case please start manually!"
                )
                dummy_progress(injection_progressbar)
                ...
                if rgl_path is not None:
                    try:
                        ...
                        logger.error(
                            f"Failed to run GTA5. Traceback: {err}", exc_info=1
                        )
                        inject_progress_label.configure(
                            text="Failed to run GTAV using Rockstar Games Launcher!",
                            text_color=RED,
                        )
                        pass
                else:
                    inject_progress_label.configure(
                        text="Could not find Rockstar Games version of GTA!\nAre you sure your game uses Rockstar Launcher?\nTry choosing a different option instead.",
                        text_color=YELLOW,
                    )
                sleep(3)
            elif run_cmd == "_none":
                inject_progress_label.configure(
                    text="Please select your lancher from the dropdown list!",
                    text_color=RED,
                )
                start_gta_button.configure(state="disabled")
            else:
                inject_progress_label.configure(
                    text=f"Please wait while YMU attempts to launch your game through\n{user_launcher}..."
                )
                dummy_progress(injection_progressbar)
                start_gta_button.configure(state="disabled")
            ...
            inject_progress_label.configure(
                text=f"Error finding a GTA 5 executable\nERROR: {e}", text_color=RED
            )
            reset_inject_progress_label(10)
            start_gta_button.configure(state="normal")
    else:
        inject_progress_label.configure(
            text="GTA 5 is already running!", text_color=RED
        )
        reset_inject_progress_label(10)
...
step_indicator_label = ctk.CTkLabel(
    master=buttons_frame, text=s_i_str, font=CODE_FONT_SMALL, text_color=WHITE
)
        ...
        with open(yimSettings, "r") as jsonfile:
            ...
            if key in setting:
                if setting[key] is True:
                    ...
                    lua_ar_switch.configure(
                        text=f"Enable Auto Reload for Lua-Scripts? ({lua_ar_switch.get()})",
                        button_color=GREEN,
                        button_hover_color=GREEN_D,
                        border_color=GREEN_D,
                    )
...
def lua_auto_reload():
    ...
    yimSettings = f"{yimPath}\\settings.json"
    if (
        os.path.exists(yimPath)
        and os.path.isfile(yimSettings)
        and lua_ar_switch.get() == "ON"
    ):
        with open(yimSettings, "r") as jsonfile:
            data = json.load(jsonfile)
            setting = data["lua"]
        ...
        lua_ar_switch.configure(
            button_color=GREEN, button_hover_color=GREEN_D, border_color=GREEN_D
        )

    elif (
        os.path.exists(yimPath)
        and os.path.isfile(yimSettings)
        and lua_ar_switch.get() == "OFF"
    ):
        with open(yimSettings, "r") as jsonfile:
            data = json.load(jsonfile)
            setting = data["lua"]
        ...
        lua_ar_switch.configure(
            button_color=RED, button_hover_color=RED_D, border_color=RED_D
        )

    else:
        lua_ar_switch.configure(
            text="❌ YimMenu isn't installed!\nOr has never been injected.",
            text_color=RED,
        )
        sleep(5)
        lua_ar_switch.configure(
            text="Enable Auto Reload for Lua-Scripts?", text_color=RED
        )


luas_header = ctk.CTkLabel(
    master=settings_frame,
    text="▾ Lua Settings ▾",
    text_color=WHITE,
    font=BIG_FONT,
...
def lua_ar_switch_hover(e):
    ...
    if lua_ar_switch.get() == "ON":
        lua_ar_switch.configure(
            button_color=GREEN_D, border_color=GREEN_D, font=SMALL_BOLD_FONT_U
        )
    elif lua_ar_switch.get() == "OFF":
        lua_ar_switch.configure(
            button_color=RED_D, border_color=RED_D, font=SMALL_BOLD_FONT_U
        )
...
def lua_ar_switch_normal(e):
    ...
    if lua_ar_switch.get() == "ON":
        lua_ar_switch.configure(
            button_color=GREEN, border_color=GREEN, font=SMALL_BOLD_FONT
        )
    elif lua_ar_switch.get() == "OFF":
        lua_ar_switch.configure(
            button_color=RED, border_color=RED, font=SMALL_BOLD_FONT
        )
...
lua_info_label = ctk.CTkLabel(
    ...
    text="You have x Luas installed:",
    font=SMALL_BOLD_FONT,
    text_color=WHITE,
)
...
refresh_lua_list_button = ctk.CTkButton(
    master=lua_manage_frame,
    fg_color=GREEN,
    text="Refresh ↻",
    font=SMALL_BOLD_FONT,
...
def open_lua_info(e):
    ...
    lua_info_frame = ctk.CTkFrame(
        master=lua_info, fg_color=BG_COLOR, border_width=1, border_color=GREEN_D
    )
...
def discover_luas_hover(e):
    ...
    luas_label.configure(
        font=SMALL_BOLD_FONT_U, text="\nDon't have any Luas or want more?"
    )
    lua_settings_frame.configure(border_width=1)
    tabview.configure(border_color=GREEN)
...
def discover_luas_normal(e):
    ...
    luas_label.configure(
        font=SMALL_BOLD_FONT, text="\n↘ Don't have any Luas or want more? ↙"
    )
    lua_settings_frame.configure(border_width=0)
    tabview.configure(border_color=GREEN_D)


discover_luas_button = ctk.CTkButton(
    master=lua_settings_frame,
    fg_color=GREEN,
    text="Discover Luas ↗",
    font=SMALL_BOLD_FONT,
        ...
        with open(yimSettings, "r") as jsonfile:
            ...
            if key in setting:
                if setting[key] is True:
                    ...
                    e_console_switch.configure(
                        text=f"Enable External Debug Console? ({e_console_switch.get()})",
                        button_color=GREEN,
                        button_hover_color=GREEN_D,
                        border_color=GREEN_D,
                    )
...
def folder_button_hover(e):
    folder_button.configure(image=folder_hvr, text_color=GREEN, font=SMALL_BOLD_FONT_U)
    tabview.configure(border_color=GREEN)
    other_settings_frame.configure(border_width=1)
...
def report_bug_btn_normal(e):
    report_bug_btn.configure(
        image=report_bug_white, text_color=WHITE, font=SMALL_BOLD_FONT
    )
    tabview.configure(border_color=GREEN_D)
    other_settings_frame.configure(border_width=0)
...
def request_feature_btn_normal(e):
    request_feature_btn.configure(
        image=request_feature_white, text_color=WHITE, font=SMALL_BOLD_FONT
    )
    tabview.configure(border_color=GREEN_D)
    other_settings_frame.configure(border_width=0)
...
def on_exit():
    if os.path.isfile("./ymu/cache.sqlite"):
        os.remove("./ymu/cache.sqlite")
    else:
        pass
    if os.path.isfile("./ymu/cache/ymu.log.1"):
        os.remove("./ymu/ymu.log.1")
    else:
        pass
    if os.path.isfile("./ymu/cache.sqlite") or os.path.isfile("./ymu/cache/ymu.log.1"):
        logger.error(
            "Failed to delete temporary cache or old logs!\nThis should have no impact on YMU's usability though."
        )
    else:
        logger.info("Successfully deleted the temporary cache and old logs.")
