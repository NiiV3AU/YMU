...
try:
    ymu_version = update_checker.LOCAL_VERSION
except Exception:
...
STYLESHEET = """
    ...
    /* Style for the popup list items */
    QComboBox QAbstractItemView {
        background-color: #2C2C2C;
        border: 1px solid #555555;
        border-radius: 8px;
        selection-background-color: #28A745; /* @primary */
        color: #E0E0E0; /* @text-primary */
        padding: 5px;
        outline: 0px;
    }
...
STYLESHEET_LIGHT = """
    ...
    /* --- GLOBAL STYLES --- */
    QWidget { color: #121212; font-family: "Manrope"; font-size: 14px;}
    ...
    /* --- ComboBox Styling --- */
    QComboBox { background-color: #E0E0E0; border: 1px solid #DCDCDC; border-radius: 8px; padding: 8px 12px; color: #121212; }
    QComboBox:hover { border-color: #BDBDBD; }
    QComboBox::drop-down { border: none; background: transparent; padding-right: 10px; }
    QComboBox::down-arrow { image: url({ASSET_PATH}/chevron-down.svg); width: 20px; height: 20px; padding-right: 10px;}
    QComboBox QAbstractItemView { background-color: #FFFFFF; border: 1px solid #CCCCCC; selection-background-color: #28A745; selection-color: #FFFFFF; color: #121212; padding: 5px; outline: 0px; border-radius: 8px;}
...
class NotificationWidget(QFrame):
    ...
    def __init__(
        self, title, message, icon_type="info", theme_manager=None, parent=None
    ):
...
class NotificationManager(QObject):
    ...
    def show(self, title, message, icon_type="info", duration=6000):
        ...
        notification = NotificationWidget(
            title, message, icon_type, self.theme_manager, self.parent_widget
        )
        notification.show()
    ...
    def _reposition_notifications(self, is_new=False, new_duration=0):
        ...
        for i, notification in enumerate(reversed(self.notifications)):
            current_y -= notification.height() + self.padding / 2
...
class InfoDialog(QDialog):
    """A reusable, modal dialog for showing information in tabs."""
    ...
    def __init__(self, title: str, content: dict, parent=None):
        ...
        for tab_title, tab_content in content.items():
            ...
            content_label = QLabel(tab_content)
...
class MainWindow(QMainWindow):
    def __init__(self, theme_manager: "ThemeManager", worker_manager: "WorkerManager"):
        super().__init__()
        ...
        self.setFixedSize(QSize(650, 440))
        ...
        self.risk_page = RiskPage(theme_manager=self.theme_manager)
        self.download_page = DownloadPage(
            theme_manager=self.theme_manager, worker_manager=self.worker_manager
        )
    ...
    def setup_sidebar(self, layout: QVBoxLayout):
        ...
        btn_risks.setToolTip("Show important warnings and information")
        ...
        footer_button.setToolTip("Open the YMU project page in your browser")
...
class RiskPage(QWidget):
    ...
    def __init__(self, theme_manager, parent=None):
        ...
        yimv2_repo_button.setToolTip("Open the YimMenuV2 (enhanced) GitHub repository")
        ...
        fsl_thread_button.clicked.connect(
            lambda: webbrowser.open("YOUR_PLACEHOLDER_LINK_HERE")
        )
        ...
        main_layout.addWidget(card_frame, 0, Qt.AlignmentFlag.AlignCenter)
        main_layout.addStretch()
...
class DownloadPage(QWidget):
    ...
    def __init__(self, theme_manager, worker_manager, parent=None):
        super().__init__(parent)
        self.theme_manager = theme_manager
        self.worker_manager = worker_manager
        ...
        info_button.setToolTip("Show help for DLL and FSL installation")
        info_button.setIconSize(QSize(20, 20))

        self.channel_select = QComboBox()
        self.channel_select.addItems(list(self.RELEASE_CHANNELS.keys()))
        self.channel_select.setToolTip("Select the YimMenu version to download")
        self.channel_select.setCursor(Qt.CursorShape.PointingHandCursor)

        self.status_label = QLabel("Select a channel to check for updates.")
        self.status_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.download_button = AnimatedButton(
            "Check for Updates", theme_manager=self.theme_manager
        )
        self.download_button.setFixedSize(140, 40)
    ...
    def _on_download_button_clicked(self):
        ...
        button_text = self.download_button.text()
        if button_text in ["Download", "Update"]:
            self.start_download()
        else:
            self.trigger_update_check()
    ...
    def trigger_update_check(self):
        ...
        self.download_button.setText("Checking...")
    ...
    def show_download_info_dialog(self):
        ...
        fsl_info_text = (
            ...
            "3. Drop the version.dll in the folder\n"
    ...
    def _update_check_logic(self, progress_signal=None):
        """
        Fetches the latest release data, using a cache to avoid spamming the API.
        """
    ...
    def _handle_update_check_result(self, status: str):
        if status == "Up-to-date":
            self.status_label.setText("YimMenu is up-to-date.")
            self.download_button.setText("Up-to-date")
            self.download_button.setEnabled(False)
        elif status in ["Download", "Update"]:
            self.status_label.setText("A new version is available!")
            self.download_button.setText(status)
            self.download_button.setEnabled(True)
            cast(MainWindow, self.window()).notification_manager.show(
                f"{self.channel_select.currentText()} Update",
    ...
    def _handle_worker_error(self, error: Exception):
        ...
        self.download_button.setText("Retry Check")
        self.download_button.setEnabled(True)
        cast(MainWindow, self.window()).notification_manager.show(
            "Download Error",
    ...
    def start_download(self):
        ...
        self.status_label.setText(
            f"Downloading {self.latest_release_data.asset_name}..."
        )
        self.download_button.setEnabled(False)
    ...
    def _handle_download_result(self, success: bool):
        if success:
            ...
            self.status_label.setText("Download successful and verified!")
            cast(MainWindow, self.window()).notification_manager.show(
                "Download Complete",
    ...
    def _set_to_uptodate_state(self):
        """Helper-Methode, um den finalen UI-Zustand zu setzen."""
        self.download_button.reset_progress()
        self.download_button.setText("Up-to-date")
        self.download_button.setEnabled(False)
...
class InjectPage(QWidget):
    ...
    def __init__(self, theme_manager, worker_manager, parent=None):
        ...
        info_button.setObjectName("InfoButton")
        info_button.setToolTip("Show help for the injection process")
        info_button.setIconSize(QSize(20, 20))

        self.launcher_select = QComboBox()
        self.launcher_select.addItems(
            ["Select Launcher", "Steam", "Epic Games", "Rockstar Games"]
        )
        self.launcher_select.setFixedWidth(150)
        self.launcher_select.setToolTip("Select the launcher you use to start GTA V")
        self.launcher_select.setCursor(Qt.CursorShape.PointingHandCursor)

        self.dll_select = QComboBox()
        self.dll_select.setToolTip("Select the DLL to inject")
        ...
        self.start_gta_button = AnimatedButton(
            "Start GTA 5", theme_manager=self.theme_manager
        )
        self.start_gta_button.setEnabled(False)
        self.inject_button = AnimatedButton(
            "Inject YimMenu", theme_manager=self.theme_manager
        )
    ...
    def _update_dll_selector(self):
        ...
        if len(found_dlls) == 0:
            self.dll_select.setVisible(False)
            self.dll_to_inject = None
            self.inject_button.setText("No DLL found")
            self.inject_button.setEnabled(False)
        elif len(found_dlls) == 1:
            self.dll_select.setVisible(False)
            self.dll_to_inject = found_dlls[0]
            self.inject_button.setText(f"Inject {cleaned_names[0]}")
        else:
            ...
            self.inject_button.setText(f"Inject {current_cleaned_name}")
    ...
    def _on_dll_selection_changed(self, index):
        ...
        if index > -1:
            ...
            self.inject_button.setText(f"Inject {cleaned_name}")
    ...
    def show_inject_info_dialog(self):
        ...
        content = {"Start GTA 5": start_gta_text, "Inject DLL": inject_text}
        dialog = InfoDialog(title="Injection Info", content=content, parent=self)
        dialog.exec()
    ...
    def handle_start_gta_click(self):
        ...
        if process_manager.find_gta_pid() is not None:
            cast(MainWindow, self.window()).notification_manager.show(
                "Information", "GTA 5 is already running!", icon_type="info"
            )
            return
        if self.launcher_select.currentIndex() == 0:
            cast(MainWindow, self.window()).notification_manager.show(
                "Error", "Please select a launcher first.", icon_type="error"
            )
            return
    ...
    def _get_rockstar_path(self) -> str | None:
        """Finds the GTA V install path for the RGL version from the registry."""
        if not IS_WINDOWS:
            return None
        try:
            ...
            path, _ = winreg.QueryValueEx(regkey, r"InstallFolder")
            winreg.CloseKey(regkey)
            return path
        except OSError as e:
            logger.error(f"Could not read registry for RGL path: {e}")
            return None
    ...
    def _launch_game_logic(self, progress_signal=None):
        ...
        if uri:
            ...
            if not path:
                msg = "Could not find Rockstar Games installation path in registry."
                logger.error(msg)
                raise FileNotFoundError(msg)

            executable_path = os.path.join(path, "PlayGTAV.exe")
            if not os.path.exists(executable_path):
                msg = f"Executable not found at '{executable_path}'."
                logger.error(msg)
                raise FileNotFoundError(msg)

            try:
                os.startfile(executable_path)
                logger.info("Successfully launched PlayGTAV.exe.")
                return "Successfully started GTA 5 via Rockstar Launcher."
            except OSError as e:
                logger.exception(f"Failed to start PlayGTAV.exe: {e}")
                raise RuntimeError("Error launching game. See logs for details.") from e
        else:
            msg = f"Attempted to launch with an unknown launcher: {launcher}"
            logger.error(msg)
    ...
    def _inject_logic(self, progress_signal=None):
        ...
        if not self.dll_to_inject:
            raise ValueError("Error: No DLL selected or found for injection.")

        if not process_manager.is_process_running(self.gta_pid):
            raise RuntimeError("GTA 5 process disappeared before injection.")

        success = process_manager.inject_dll(self.gta_pid, self.dll_to_inject)

        if success:
            return f"Successfully injected '{self.dll_to_inject}'!"
        else:
            raise RuntimeError(
                f"Injection of '{self.dll_to_inject}' failed. See logs for details."
            )
...
class SettingsPage(QWidget):
    def __init__(self, theme_manager, worker_manager, parent=None):
        ...
        lua_title = QLabel("Lua Settings")
        lua_title.setObjectName("SettingsTitle")
        auto_reload_layout = QHBoxLayout()
        self.auto_reload_label = QLabel("Auto-reload changed scripts")
        ...
        btn_enable_script.setToolTip("Enable selected script(s)")
        ...
        self.btn_refresh_luas.setToolTip("Refresh script lists")
        ...
        btn_disable_script.setToolTip("Disable selected script(s)")
        ...
        self.debug_console_label = QLabel("Enable External Debug Console")
        ...
        btn_open_folder.setToolTip("Open YimMenu folder (%APPDATA%/YimMenu)")
        ...
        btn_open_ymu_folder.setToolTip("Open YMU folder (%APPDATA%/YMU)")
        ...
        btn_report_bug.setToolTip("Open the bug report page on GitHub in your browser")
        ...
        self.btn_check_for_updates = AnimatedButton(
            "Check for YMU Updates", theme_manager=self.theme_manager
        )
    ...
    def _on_update_check_finished(self, result):
        """Displays the result of the update check and prompts the user if needed."""
        ...
        if status == "UP_TO_DATE":
            cast(MainWindow, self.window()).notification_manager.show(
                "YMU Updater",
                "Your YMU is already up-to-date.",
                icon_type="success",
            )
            self.btn_check_for_updates.setText("YMU is up-to-date")
            QTimer.singleShot(
                5000,
                lambda: self.btn_check_for_updates.setText("Check for YMU Updates"),
            )
    ...
    def _start_updater_download(self):
        ...
        self._is_task_running = True
        self.btn_check_for_updates.setEnabled(False)
        self.btn_check_for_updates.setText("Downloading Updater...")
    ...
    def _on_updater_download_finished(self, result):
        """Called when the updater download is complete. Exits the application."""
        ...
        if success:
            ...
            self.btn_check_for_updates.setText("Check for YMU Updates")
            self.btn_check_for_updates.setEnabled(True)
            self._is_task_running = False
            cast(MainWindow, self.window()).notification_manager.show(
                "Update Error", message, icon_type="error"
            )
    ...
    def _on_task_error(self, error: Exception):
        ...
        cast(MainWindow, self.window()).notification_manager.show(
            "Error", f"An unexpected error occurred: {error}", icon_type="error"
        )
...
if __name__ == "__main__":
    ...
    window = MainWindow(theme_manager=theme_manager, worker_manager=worker_manager)
